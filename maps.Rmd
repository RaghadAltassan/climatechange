...
# تحميل بيانات CO2 من الراستر واستخراج المتوسط لكل مدينة
co2_2018_r <- terra::rast("/Users/raghadaltassan/Desktop/senior project/data/co218.tif")
co2_2019_r <- terra::rast("/Users/raghadaltassan/Desktop/senior project/data/co219.tif")
co2_2020_r <- terra::rast("/Users/raghadaltassan/Desktop/senior project/data/co220.tif")
co2_2021_r <- terra::rast("/Users/raghadaltassan/Desktop/senior project/data/co221.tif")

saudi_sf$co2_2018 <- terra::extract(co2_2018_r, saudi_sf, fun = mean, na.rm = TRUE)[, 2]
saudi_sf$co2_2019 <- terra::extract(co2_2019_r, saudi_sf, fun = mean, na.rm = TRUE)[, 2]
saudi_sf$co2_2020 <- terra::extract(co2_2020_r, saudi_sf, fun = mean, na.rm = TRUE)[, 2]
saudi_sf$co2_2021 <- terra::extract(co2_2021_r, saudi_sf, fun = mean, na.rm = TRUE)[, 2]

# تحديد كل القيم لمقياس الألوان
all_vals <- c(saudi_sf$co2_2018, saudi_sf$co2_2019, saudi_sf$co2_2020, saudi_sf$co2_2021)
pal <- colorNumeric(palette = c("#DEEBF7", "#3182BD"), domain = all_vals)

# إنشاء خريطة تفاعلية
leaflet(data = saudi_sf) %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
    fillColor = ~pal(co2_2018), color = "white", weight = 1, fillOpacity = 0.7,
    group = "2018",
    popup = ~paste0(NAME_2, ": ", formatC(co2_2018, format = "f", digits = 7), " ppm"),
    label = ~paste0(NAME_2, ": ", formatC(co2_2018, format = "f", digits = 7), " ppm"),
    labelOptions = labelOptions(textsize = "12px")
  ) %>%
  addPolygons(
    fillColor = ~pal(co2_2019), color = "white", weight = 1, fillOpacity = 0.7,
    group = "2019",
    popup = ~paste0(NAME_2, ": ", formatC(co2_2019, format = "f", digits = 7), " ppm"),
    label = ~paste0(NAME_2, ": ", formatC(co2_2019, format = "f", digits = 7), " ppm"),
    labelOptions = labelOptions(textsize = "12px")
  ) %>%
  addPolygons(
    fillColor = ~pal(co2_2020), color = "white", weight = 1, fillOpacity = 0.7,
    group = "2020",
    popup = ~paste0(NAME_2, ": ", formatC(co2_2020, format = "f", digits = 7), " ppm"),
    label = ~paste0(NAME_2, ": ", formatC(co2_2020, format = "f", digits = 7), " ppm"),
    labelOptions = labelOptions(textsize = "12px")
  ) %>%
  addPolygons(
    fillColor = ~pal(co2_2021), color = "white", weight = 1, fillOpacity = 0.7,
    group = "2021",
    popup = ~paste0(NAME_2, ": ", formatC(co2_2021, format = "f", digits = 7), " ppm"),
    label = ~paste0(NAME_2, ": ", formatC(co2_2021, format = "f", digits = 7), " ppm"),
    labelOptions = labelOptions(textsize = "12px")
  ) %>%
  addLegend(pal = pal, values = all_vals, title = "CO₂ (ppm)", position = "bottomright") %>%
  addLayersControl(
    baseGroups = c("2018", "2019", "2020", "2021"),
    options = layersControlOptions(collapsed = FALSE)
  )
...
pal_ch4 <- colorNumeric(palette = c("#DEEBF7", "#3182BD"), domain = pretty_vals)
